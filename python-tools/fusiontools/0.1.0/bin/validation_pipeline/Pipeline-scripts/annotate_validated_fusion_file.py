...
parser.add_argument('output_file', action='store', help='validated fusion output file, annotated with candidate gene names and strand information')
...
def map_validated_fusions_to_known_genes(validated_fusions_file_name, gene_ann_bed):
    ...
    for fusion in validated_fusions:
        # check whole .bed file for candidade gene mappings, add "chr" to account for possibility that validation file does not have "chr" prefix for chromosomes
        for genebed in genebed_objects:
            if ( ( genebed.start <= fusion.start1 <= genebed.end) or (genebed.start <= fusion.end1 <= genebed.end) ) and ( (str(fusion.chr1)) == genebed.chr or ("chr"+str(fusion.chr1)) == genebed.chr ):
            ...
            if ( ( genebed.start <= fusion.start2 <= genebed.end) or (genebed.start <= fusion.end2 <= genebed.end) ) and ( (str(fusion.chr2)) == genebed.chr or ("chr"+str(fusion.chr2)) == genebed.chr ):
...
new_validated_fusions_file = open(args.output_file, 'w+')
...
for gene_pair,validated_fusion in zip(gene_pairs, validated_fusions):
    ...
    validated_fusion.gene1_candidates = "|".join([gene[0] for gene in left_mapped_genes]) if left_mapped_genes else "NA"
    validated_fusion.gene2_candidates = "|".join([gene[0] for gene in right_mapped_genes]) if right_mapped_genes else "NA"
    validated_fusion.gene1_strands += "|".join([gene[2] for gene in left_mapped_genes]) if left_mapped_genes else "NA"  
    validated_fusion.gene2_strands += "|".join([gene[2] for gene in right_mapped_genes]) if right_mapped_genes else "NA"
    ...
    # CAN CHANGE COMMENT TO INCLUDE SAMPLE NAME
