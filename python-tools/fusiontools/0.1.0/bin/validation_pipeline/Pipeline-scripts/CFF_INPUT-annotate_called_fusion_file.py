...
parser.add_argument('cff_file', action='store', help='A file containing a set of called fusions')
parser.add_argument('sample_name', action='store', help='Name of the sample being called')
...
def map_called_fusions_to_known_genes(called_fusions_file_name, gene_ann_bed):
    ...
    for fusion in called_fusions:
        ...
        for genebed in genebed_objects:
            #print("genebed.start", genebed.start)
            #print("genebed.chr", genebed.chr)
            #print(fusion.pos1)
            if ( genebed.start <= fusion.pos1 <= genebed.end  ) and (fusion.chr1 == genebed.chr):
            ...
            if (  genebed.start <= fusion.pos2 <= genebed.end)  and (fusion.chr2 == genebed.chr):
...
gene_pairs, called_fusions = map_called_fusions_to_known_genes(args.cff_file, known_genes_file_name) 
