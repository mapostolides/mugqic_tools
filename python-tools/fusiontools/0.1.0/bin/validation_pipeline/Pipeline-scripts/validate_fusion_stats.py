    ...
    def find_true_positives_using_breakpoints(self):
        ...
        for fusion in self.validated_fusions:
            for category_fusion in self.category_list:
                ...
                # generate list of breakpoints within "slop" of all breakpoints for a given gene
                if ( ( fusion.start1 in itertools.chain(*[range(bp-slop, bp+slop) for bp in category_fusion.breakpoint_1] )) and 
                    ( fusion.start2 in itertools.chain(*[range(bp-slop, bp+slop) for bp in category_fusion.breakpoint_2] ) ) ):
